...
std::optional<OperationValue> OperationValuesLookup::parse(std::string crude_operation){
...
uint32_t Operation::get_value() const{
	return (uint32_t)value;
}

bool Operation::can_be_first() const {
	return true;
}
...
bool Number::can_be_first() const {
	return false;
}
